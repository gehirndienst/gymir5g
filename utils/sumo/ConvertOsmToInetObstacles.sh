...
netBoundaries=$(pcregrep -o1 --buffer-size=9999999 "convBoundary=\"([^\"]*)\"" $netFile | sed 's/,/ /g')
read netBounds1X netBounds1Y netBounds2X netBounds2Y <<< "$netBoundaries"

# Take the boundaries from the poly file.
polyBoundaries=$(pcregrep -o1 --buffer-size=9999999 "convBoundary=\"([^\"]*)\"" $polyFile)

# Take the shape of the buildings.
shape=$(egrep "^ *<poly.*type=\"(amenity|building)\.?[^\"]*\"" $polyFile |
        pcregrep -o1 --buffer-size=9999999 "shape=\"([^\"]*)\"" | sed 's/,/ /g')
